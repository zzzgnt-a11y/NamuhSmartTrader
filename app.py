...
try:
    ...
    AUTO_LOOP_SECONDS=5.0
LOOP_STATE={"started_at":0.0,"last_tick":0.0,"last_ok":0.0,"last_error":"","iterations":0}
COIN_SETTINGS_LOCK=threading.RLock()
COIN_SETTINGS={"explicit_budget":None,"auto_max_if_unset":True,"auto_trade_enabled":True,"entry_score":66.0}
COIN_COOLDOWN={}
COIN_LOOP_STATE={"started_at":0.0,"last_tick":0.0,"last_ok":0.0,"last_error":"","iterations":0}
...
def _persist_coin_settings():
    with COIN_SETTINGS_LOCK:
        payload={"settings":dict(COIN_SETTINGS),"cooldown":dict(COIN_COOLDOWN)}
    store.save_json("coin_settings",payload)
...
def _restore_coin_settings():
    ...
    with COIN_SETTINGS_LOCK:
        if settings.get("explicit_budget") is None:
            COIN_SETTINGS["explicit_budget"]=None
        else:
            try:COIN_SETTINGS["explicit_budget"]=max(0,int(settings.get("explicit_budget")))
            except Exception:pass
        COIN_SETTINGS["auto_max_if_unset"]=bool(settings.get("auto_max_if_unset",COIN_SETTINGS["auto_max_if_unset"]))
        COIN_SETTINGS["auto_trade_enabled"]=bool(settings.get("auto_trade_enabled",COIN_SETTINGS["auto_trade_enabled"]))
        try:COIN_SETTINGS["entry_score"]=max(50.0,min(90.0,float(settings.get("entry_score",COIN_SETTINGS["entry_score"]))))
        except Exception:pass
        cd=data.get("cooldown") if isinstance(data.get("cooldown"),dict) else {}
        now=time.time();COIN_COOLDOWN.clear()
        for k,v in cd.items():
            try:
                ts=float(v)
                if now-ts<86400:COIN_COOLDOWN[str(k).upper()]=ts
            except Exception:pass

def _coin_settings_snapshot():
    with COIN_SETTINGS_LOCK:return dict(COIN_SETTINGS)

def _coin_effective_budget():
    settings=_coin_settings_snapshot();equity=max(0.0,float(coin_paper.equity_krw()))
    explicit=settings.get("explicit_budget")
    if explicit is not None:return min(equity,max(0.0,float(explicit)))
    return equity if settings.get("auto_max_if_unset",True) else 0.0

def _coin_available_budget():
    return max(0.0,min(float(coin_paper.cash_krw),_coin_effective_budget()-float(coin_paper.held_cost_krw())))
...
def ai_loop():
    LOOP_STATE["started_at"]=time.time();last_persist=0.0
    while True:
        LOOP_STATE["last_tick"]=time.time();LOOP_STATE["iterations"]+=1
        try:
            ...
            if time.time()-last_persist>=60:
                _persist_paper();last_persist=time.time()
            LOOP_STATE["last_ok"]=time.time();LOOP_STATE["last_error"]=""
        except Exception as exc:
            LOOP_STATE["last_error"]=str(exc)[:300]
            print("AI LOOP ERROR:",exc,flush=True)
        time.sleep(AUTO_LOOP_SECONDS)

def coin_trade_loop():
    COIN_LOOP_STATE["started_at"]=time.time();last_persist=0.0
    while True:
        COIN_LOOP_STATE["last_tick"]=time.time();COIN_LOOP_STATE["iterations"]+=1
        try:
            candidates=coin_feed.candidates(50);score_map={x.get("code"):float(x.get("score",0) or 0) for x in candidates}
            changed=False
            for p in list(coin_paper.positions.values()):
                q=coin_feed.quote(p.symbol)
                if not q or q.price<=0:continue
                coin_paper.mark(p.symbol,q.price)
                score=score_map.get(p.symbol,50.0);reason=""
                if p.pnl_pct>=3.0:reason="목표수익 +3% 도달"
                elif p.pnl_pct<=-1.5:reason="손절 기준 도달"
                elif score<46:reason="AI 점수 이탈"
                if reason and coin_paper.sell(p.symbol,q.price,reason):
                    COIN_COOLDOWN[p.symbol]=time.time();changed=True
            settings=_coin_settings_snapshot()
            if settings.get("auto_trade_enabled",True):
                entry=float(settings.get("entry_score",66) or 66)
                for item in candidates:
                    if float(item.get("score",0) or 0)<entry:break
                    symbol=str(item.get("code") or "").upper()
                    if not symbol or f"COIN:{symbol}" in coin_paper.positions:continue
                    if float(item.get("fresh_age",9999) or 9999)>30:continue
                    if time.time()-float(COIN_COOLDOWN.get(symbol,0) or 0)<300:continue
                    q=coin_feed.quote(symbol);available=_coin_available_budget();budget=_coin_effective_budget()
                    if not q or q.price<=0 or available<10000:continue
                    spend=min(available,max(10000.0,budget*0.20))
                    if coin_paper.buy(q,spend,"COIN_SCALP"):
                        changed=True;break
            if changed:
                _persist_coin();_persist_coin_settings()
            if time.time()-last_persist>=60:
                _persist_coin();_persist_coin_settings();last_persist=time.time()
            COIN_LOOP_STATE["last_ok"]=time.time();COIN_LOOP_STATE["last_error"]=""
        except Exception as exc:
            COIN_LOOP_STATE["last_error"]=str(exc)[:300]
            print("COIN LOOP ERROR:",exc,flush=True)
        time.sleep(AUTO_LOOP_SECONDS)
...
def start_background():
    ...
    threading.Thread(target=coin_trade_loop,daemon=True).start()
    events.start()
...
@app.get("/coin/{symbol}")
def coin_detail_page(symbol:str):
    if not re.fullmatch(r"[A-Za-z0-9._-]{1,20}",str(symbol)):raise HTTPException(404)
...
def coin_account_state():
    ...
    equity=coin_paper.equity_krw();settings=_coin_settings_snapshot();budget=_coin_effective_budget();available=_coin_available_budget()
    pnl=equity-coin_paper.initial_cash_krw
    return {"initial_cash":coin_paper.initial_cash_krw,"cash":krw(coin_paper.cash_krw),"equity":krw(equity),
            "pnl":krw(pnl),"pnl_pct":(pnl/coin_paper.initial_cash_krw*100 if coin_paper.initial_cash_krw else 0.0),
            "budget":krw(budget),"effective_budget":krw(budget),"available_budget":krw(available),
            "explicit_budget":settings.get("explicit_budget"),"auto_max_if_unset":settings.get("auto_max_if_unset",True),
            "auto_trade_enabled":settings.get("auto_trade_enabled",True),"entry_score":settings.get("entry_score",66.0),
            ...
            "unrealized_pnl":krw(coin_paper.unrealized_pnl_krw()),"total_pnl":krw(pnl),"exchange":"Coinone"}
...
def set_coin_settings(data:CoinSettingsRequest):
    equity=max(0,int(round(coin_paper.equity_krw())))
    if data.amount is not None and (data.amount<0 or data.amount>equity):
        raise HTTPException(400,f"코인 운용금액은 0~{equity}원 범위입니다")
    if not (50<=float(data.entry_score)<=90):raise HTTPException(400,"진입 기준점수는 50~90 사이입니다")
    with COIN_SETTINGS_LOCK:
        COIN_SETTINGS["explicit_budget"]=None if data.amount is None else int(data.amount)
        COIN_SETTINGS["auto_max_if_unset"]=bool(data.auto_max_if_unset)
        COIN_SETTINGS["auto_trade_enabled"]=bool(data.auto_trade_enabled)
        COIN_SETTINGS["entry_score"]=float(data.entry_score)
    _persist_coin_settings()
    return {"ok":True,"settings":_coin_settings_snapshot(),"account":coin_account_state()}

def _coin_market_rows(n=30):
    out=[]
    for q in coin_feed.top_quotes(n):
        out.append({"market":"COIN","code":q.symbol,"name":q.name or q.symbol,"price":q.price,
...
def coin_state():
    return {"ok":True,"market":_coin_market_rows(30),"candidates":coin_feed.candidates(30),
            "account":coin_account_state(),"settings":_coin_settings_snapshot(),
            "health":coin_feed.health(),"build":BUILD_ID}

@app.get("/api/coin/chart/{symbol}")
def coin_chart(symbol:str,interval:str=Query("1d"),size:int=Query(180,ge=20,le=500)):
    symbol=str(symbol).upper();q=coin_feed.quote(symbol)
    if not q:
    ...
    return {"market":"COIN","exchange":"Coinone","code":symbol,"name":q.name or symbol,"price":q.price,
            "change_pct":q.change_pct,"quote_volume":q.quote_volume,"target_volume":q.target_volume,
            "volume_power":q.volume_power,"spread_pct":q.spread_pct,"book_imbalance":q.book_imbalance,
            "interval":interval,"bars":bars,"settings":_coin_settings_snapshot(),
...
def coin_detail(symbol:str,interval:str=Query("1m"),size:int=Query(120,ge=20,le=500)):
    ...
    return {"market":"COIN","exchange":"Coinone","code":symbol,"name":q.name or symbol,
            ...
            "source":"Coinone Public API","real_orders_enabled":False,"settings":_coin_settings_snapshot(),"build":BUILD_ID}
