(function(){
  ...
  function buildEventBrowser(){
    const panel=q('.event-panel'),base=q('#eventList');if(!panel||!base)return null;let box=q('#v344EventBrowser');if(box)return box;
  ...
  function useLiveEventFallback(message){const base=q('#eventList'),box=q('#v344EventBrowser');const hasLive=Boolean(base?.querySelector('.event-item,[data-stock]'));if(hasLive){base.style.display='';if(box)box.style.display='none';return true}if(base)base.style.display='none';if(box)box.style.display='flex';const st=q('#v344EventStatus');if(st)st.textContent=message||'공식 공시 연결 대기';return false}
  function eventRow(e){
