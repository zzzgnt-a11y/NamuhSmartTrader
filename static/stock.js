...
function draw(){
  ...
  const v=visibleBars();if(!v.length){ctx.fillStyle="#8498b3";ctx.font="12px sans-serif";ctx.fillText("차트 데이터 축적 중",20,30);return}
...
function render(d){
  ...
  $("chartStatus").textContent=bars.length?`${TF.toUpperCase()} · ${bars.length}개 데이터 · 서버 수신 공식 데이터`:"공식 데이터 축적 중";
  const labels={"1m":"1분","3m":"3분","5m":"5분","20m":"20분","1d":"1D"};
  ...
  const a=d.analysis;$("analysisReasons").innerHTML=a?(a.reasons||[]).slice(0,10).map(x=>`<span>${esc(x)}</span>`).join(""):'<div class="empty">해당 시간대 기준 AI 점수 데이터 축적 중</div>';
  ...
  $("stockEvents").innerHTML=(d.events||[]).length?d.events.map(e=>`<div class="event-item stock-event-item"><div class="event-head"><span class="event-label ${e.sentiment==="negative"?"negative":""}">${esc(e.label)}</span><small>${esc(e.date)} · ${esc(e.source||"DART 공식")}</small></div><b>${esc(e.title)}</b><div class="event-ai-summary"><strong>AI 요약</strong><span>${esc(disclosureSummary(e))}</span></div>${e.url?`<a href="${esc(e.url)}" target="_blank" rel="noopener">공시 원문 ↗</a>`:""}</div>`).join(""):'<div class="empty">해당 종목의 최근 공시 없음 또는 DART 연결 대기</div>';
  resize()
}
