(function(){
  ...
  function cleanCoinHero(){q('.coin-hero-unified .hero-copy > p')?.remove();q('.coin-hero-unified .session-pill')?.remove()}
  ...
  window.tradeRow=function(t){
    ...
    const sidePrice=sell?(t.sell_price??t.price):(t.buy_price??t.price);
    const sideAmount=sell?(t.sell_amount??t.gross_krw):(t.buy_amount??t.gross_krw);
    const sideLabel=sell?'매도가':'매수가',amountLabel=sell?'매도금액':'매수금액';
    return `<article class="v34-trade-card"><div class="v34-trade-title"><span class="trade-side ${sell?'sell':'buy'}">${sell?'SELL':'BUY'}</span><div><b>${esc(t.name||t.code)}</b><small>${esc(t.code)} · ${esc(tradeStamp(t))}</small></div><strong>${qty}<small>개</small></strong></div><div class="v34-trade-grid v342-trade-grid"><div><span>수량</span><b>${qty}개</b></div><div><span>${sideLabel}</span><b>${fmt(sidePrice)}</b></div><div><span>${amountLabel}</span><b>${won(sideAmount)}</b></div></div><div class="v34-trade-pnl"><span>${esc(t.reason||t.strategy||'')}</span><b class="${Number(t.pnl||0)>=0?'pos':'neg'}">${sell?won(t.pnl)+' · '+pct(t.pnl_pct):'매수 완료'}</b></div></article>`;
  };
  ...
  function init(){addWithAlice();cleanCoinHero();topAuto();normalizeText();const obs=new MutationObserver(()=>{cleanCoinHero();normalizeText()});['#coinMarketGrid','#coinCandidateList','#coinTrades'].forEach(s=>{const el=q(s);if(el)obs.observe(el,{childList:true,subtree:true})});setInterval(load,12000);setInterval(()=>{cleanCoinHero();normalizeText()},1500)}
