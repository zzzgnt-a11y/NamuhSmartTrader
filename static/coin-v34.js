(function(){
  ...
  window.tradeRow=function(t){
    const sell=String(t.side||'').toUpperCase()==='SELL',qty=Number(t.qty||0).toLocaleString(undefined,{maximumFractionDigits:8});
    ...
    return `<article class="v34-trade-card"><div class="v34-trade-title"><span class="trade-side ${sell?'sell':'buy'}">${sell?'SELL':'BUY'}</span><div><b>${esc(t.name||t.code)}</b><small>${esc(t.code)} · ${esc(t.date)} ${esc(t.time)}</small></div><strong>${qty}<small>개</small></strong></div><div class="v34-trade-grid"><div><span>체결가</span><b>${fmt(t.price)}</b></div><div><span>수량</span><b>${qty}개</b></div><div><span>매수가</span><b>${buyPrice==null?'-':fmt(buyPrice)}</b></div><div><span>매도가</span><b>${sellPrice==null?'-':fmt(sellPrice)}</b></div><div><span>매수금액</span><b>${buyAmount==null?'-':won(buyAmount)}</b></div><div><span>매도금액</span><b>${sellAmount==null?'-':won(sellAmount)}</b></div></div><div class="v34-trade-pnl"><span>${esc(t.reason||t.strategy||'')}</span><b class="${Number(t.pnl||0)>=0?'pos':'neg'}">${sell?won(t.pnl)+' · '+pct(t.pnl_pct):'매수 완료'}</b></div></article>`;
  };
