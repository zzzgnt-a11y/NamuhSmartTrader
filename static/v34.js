(function(){
  ...
  function buildControlStrip(){
    ...
    if(!q('#v34DisclosureAlerts')){
      const box=document.createElement('div');box.id='v34DisclosureAlerts';box.className='v34-disclosure-slot';
      strip.appendChild(box);
    }
    if(!q('#v34FlowAlerts')){
  ...
  function disclosureImpact(e){
    if(e.blocked)return '신규진입 차단';
    return Number(e.score||0)>0?'+5점':'-5점';
  }

  async function loadDisclosureAlerts(){
    const slot=q('#v34DisclosureAlerts');if(!slot)return;
    const market=currentMarket();
    try{
      const r=await fetch(`/api/v34/disclosure-alerts?market=${market}`,{cache:'no-store'});if(!r.ok)throw new Error(r.status);
      const d=await r.json(),rows=d.items||[],total=Number(d.unread_total||rows.length||0);
      if(!rows.length){slot.classList.remove('active');slot.innerHTML='';return}
      slot.classList.add('active');
      const shown=rows.slice(0,3);
      slot.innerHTML=`<div class="v342-disclosure-title"><div><small>IMPORTANT DISCLOSURE · UNREAD</small><b>미확인 중요공시 ${total}건</b></div><span>공시를 눌러 확인하기 전까지 유지 · ${d.market_open?'장중 해당 종목 집중분석':'장 시작 전까지 알림만 보관'}</span></div><div class="v342-disclosure-wrap">${shown.map(e=>`<button type="button" class="v342-disclosure-alert ${e.blocked?'blocked':e.sentiment==='positive'?'positive':'negative'}" data-v342-disclosure="${esc(e.id)}" data-v342-market="${esc(e.market)}" data-v342-code="${esc(e.code)}"><div class="v342-disclosure-top"><span>미확인</span><b>${esc(e.name||e.code)}</b><em>${esc(disclosureImpact(e))}</em></div><div class="v342-disclosure-titleline">${esc(e.title)}</div><div class="v342-disclosure-meta"><span>${esc(e.date||'')} ${esc(e.time||'')}</span><span>${esc(e.source||'')}</span><strong>${esc(e.focus_label||'다음 거래일 장중 집중분석')}</strong></div></button>`).join('')}</div>${total>3?`<div class="v342-disclosure-more">외 ${total-3}건 미확인 · 위 공시를 확인하면 다음 미확인 공시가 표시됩니다.</div>`:''}`;
      slot.querySelectorAll('[data-v342-disclosure]').forEach(b=>b.addEventListener('click',async()=>{
        const id=b.dataset.v342Disclosure||'',m=b.dataset.v342Market||market,code=b.dataset.v342Code||'';
        b.disabled=true;
        try{await fetch(`/api/v34/disclosure-alerts/${encodeURIComponent(id)}/read`,{method:'POST',cache:'no-store'})}catch(e){console.error(e)}
        if(code)location.href=`/stock/${encodeURIComponent(m)}/${encodeURIComponent(code)}`;else loadDisclosureAlerts();
      }));
    }catch(e){console.error(e)}
  }
  ...
  function watchMode(){
    const obs=new MutationObserver(()=>{updateSearchPlaceholder();keepSignalsVisible();setTimeout(loadUsEvents,50);setTimeout(loadDisclosureAlerts,80)});
  ...
  function init(){
    ...
    loadDisclosureAlerts();loadFlowAlerts();loadCachedKr();loadUsEvents();
    setInterval(loadAuto,12000);setInterval(loadDisclosureAlerts,5000);setInterval(loadFlowAlerts,3000);setInterval(loadCachedKr,7000);setInterval(()=>{keepSignalsVisible();loadUsEvents()},4000);
  }
