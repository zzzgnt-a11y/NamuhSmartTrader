(function(){
  ...
  function buildControlStrip(){
    ...
    if(!q('#v34FlowAlerts')){
      ...
      box.innerHTML='<div class="v34-alert-idle"><small>KR ABNORMAL FLOW</small><b>이상수급 실시간 감시</b></div>';
      strip.appendChild(box);
    }
  }
  ...
  async function searchStocks(raw){
    ...
    try{
      ...
      const rows=d.items||[];
      result.innerHTML=rows.length?rows.map(x=>`<button type="button" data-v34-stock="${esc(x.market)}/${esc(x.code)}"><span><b>${esc(x.name||x.code)}</b><small>${esc(x.code)} · ${esc(x.sector||'')}</small></span><em>${x.market==='US'?'$'+Number(x.price||0).toLocaleString(undefined,{maximumFractionDigits:4}):Number(x.price||0).toLocaleString('ko-KR')+'원'}</em></button>`).join(''):'<div class="v34-search-empty">검색 결과 없음</div>';
      result.classList.add('open');
      result.querySelectorAll('[data-v34-stock]').forEach(b=>b.addEventListener('click',()=>{location.href='/stock/'+b.dataset.v34Stock}));
  ...
  async function loadFlowAlerts(){
    ...
    try{
      ...
      if(!rows.length){slot.innerHTML='<div class="v34-alert-idle"><small>KR ABNORMAL FLOW</small><b>이상수급 실시간 감시</b></div>';return}
  ...
  async function enhanceCalendarDay(date){
    ...
    try{
      ...
      if(!box)return;
      box.innerHTML=row?(row.trades||[]).map(t=>`<div class="profit-row v34-profit-row"><div><b>${esc(t.name||t.code)}</b> <span class="market-badge">${esc(t.market||'')}</span><br><small>${esc(t.time||'')} · ${Number(t.qty||0).toLocaleString()}주 · 매도 ${t.market==='US'?'$'+Number(t.price||0).toLocaleString(undefined,{maximumFractionDigits:4}):Number(t.price||0).toLocaleString('ko-KR')+'원'}</small></div><div><b class="${Number(t.pnl||0)>=0?'pos':'neg'}">${won(t.pnl)}</b><br><small>${pct(t.pnl_pct)}</small></div></div>`).join(''):'<div class="empty">실현 매매 없음</div>';
    }catch(e){console.error(e)}
  }
