...
async function saveBudget() {
  ...
  const amount = parseInt(String(input?.value || '').replace(/\D/g, ''), 10) || 0;
...
function row(x, smart = false) {
  ...
  <div class="row" data-detail="${payload}">
    <div>
      ...
      <span class="pill">${(x.reasons || []).slice(0, 2).join(' · ') || '분석중'}</span>
    </div>
    ...
    <div>
      <div class="sub">${smart ? 'PER/PBR' : '점수'}</div>
      <b class="score">
        ${
          smart
    ...
    <div class="hide-sm">
      <div class="sub">${smart ? '외국인/기관' : 'VI 직전'}</div>
      <b>
        ${
          smart
...
function detail(x) {
  ...
  document.getElementById('detail').innerHTML = `
    ...
    <p><b>상승 VI 직전 참고가</b> ${won(x.vi_pre)}</p>
...
function render(s) {
  ...
  document.getElementById('health').textContent = h.nh_realtime
    ? '● NH KRX 실시간 연결'
    : '○ 연결대기 · ' +
      (h.nh_configured ? '시세 수신 대기' : 'NH API 키 미설정');

  document.getElementById('markets').innerHTML = s.market
    .map(
      (m) => `
      <div class="market">
        <b>${m.label}</b>
  ...
  document.getElementById('sectors').innerHTML = s.sectors.length
    ? s.sectors
        .map(
          (x, i) =>
            `<div class="sector">${i + 1}. ${x.sector} ${pct(x.change_pct)}</div>`
        )
        .join('')
    : `<div class="empty">KRX 데이터가 쌓이면 주도섹터가 표시됩니다.</div>`;

  document.getElementById('positions').innerHTML = p.positions.length
    ? p.positions
        .map(
          (x) => `
          <div class="row">
  ...
  document.getElementById('scalpList').innerHTML = s.scalp.length
    ? s.scalp.map((x) => row(x, false)).join('')
    : `<div class="empty">실시간 후보를 수집 중입니다.</div>`;

  document.getElementById('smartList').innerHTML = s.smart.length
    ? s.smart.map((x) => row(x, true)).join('')
    : `<div class="empty">스마트머니 후보를 수집 중입니다.</div>`;

  document.getElementById('tradeList').innerHTML = p.trades.length
    ? p.trades
        .map(
          (t) => `
          <div class="row">
            <div>
              <div class="name">${t.side === 'BUY' ? '매수' : '매도'} · ${t.name}</div>
              <div class="code">${t.time} · ${t.qty}주</div>
            </div>
            <div>
            ...
            <div>
              ...
              <b class="${t.pnl >= 0 ? 'pos' : 'neg'}">${won(t.pnl)}</b>
            </div>
            <div class="hide-sm">
...
async function refresh() {
  try {
    ...
    if (health) health.textContent = '○ 서버 연결 끊김';
    console.error(e);
  }
}
...
function bindUi() {
  ...
  document.addEventListener('click', (event) => {
    ...
    try {
      detail(JSON.parse(decodeURIComponent(el.dataset.detail)));
    } catch (e) {
      console.error(e);
    }
  });
