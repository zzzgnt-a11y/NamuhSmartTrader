...
TARGET_STOCK_ASSET_KRW = 4_000_000


def apply(ns):
    core = ns.get('core')
    if core is None or getattr(core, '_NAMUH_STOCK_ASSET_4M_APPLIED', False):
        return
    core._NAMUH_STOCK_ASSET_4M_APPLIED = True
    ...
    def restore_paper_4m():
        ...
        try:
            ...
            equity, held = 0.0, 0.0
        print(
            f'NAMUH STOCK ASSET: base=4000000 cash={paper.cash_krw:.0f} '
            f'equity={equity:.0f} held={held:.0f} budget=4000000',
            flush=True,
        )

    core._paper_payload = paper_payload_4m
    core._restore_paper = restore_paper_4m
    print('NAMUH STOCK ASSET PATCH armed: KR stock asset/budget = 4,000,000 KRW', flush=True)
