...
def _naver_stock_disclosures(code: str, months: int = 3) -> dict:
    code = str(code or "").strip(); months = max(1, min(3, int(months or 3)))
    ...
    # 1) public mobile disclosure JSON
    try:
        for page in range(1, 7):
            ...
            if not r.ok:
                break
            payload = r.json(); page_rows = []
            for d in _walk(payload):
                title = str(_pick(d, ("title", "reportName", "disclosureTitle", "subject", "noticeTitle"), "") or "").strip()
                date = _iso_date(_pick(d, ("date", "localDate", "disclosureDate", "rceptDt", "datetime"), ""))
                if not title or not date:
                    continue
                raw_url = str(_pick(d, ("url", "link", "endUrl", "disclosureUrl"), "") or "")
                if raw_url.startswith("/"):
                    raw_url = urllib.parse.urljoin("https://m.stock.naver.com", raw_url)
                page_rows.append(_event_row(code=code, title=title, date=date,
                                            source="네이버 증권 공시", url=raw_url, form="공시"))
            # de-dup recursive candidates
            uniq = {(x["date"], x["title"], x["url"]): x for x in page_rows}
            page_rows = list(uniq.values())
            items.extend(page_rows)
            if len(page_rows) < 20:
                break
    except Exception:
        pass
    # 2) public PC HTML fallback
    if not items:
        try:
    ...
    payload = {"ok": True, "market": "KR", "code": code, "months": months, "items": rows, "count": len(rows),
               "source": "네이버 증권 공개 공시 · DART키 fallback", "status": "DART_API_KEY 없음 → 네이버 공개 공시 사용",
               "updated_at": now}
...
def install(ns: dict):
    ...
    def v344_disclosures(market: str="KR", months: int=3, page: int=1, page_size: int=50):
        ...
        if _dart_key():
            try: return _dart_market_page(months,page,page_size)
            except Exception: pass
        try: return _dart_rss_recent()
        except Exception as exc: return {"ok":False,"market":"KR","items":[],"status":f"DART RSS 연결 오류: {str(exc)[:140]}","source":"DART 공식 RSS"}

    @_core.app.get("/api/v344/disclosures/{market}/{code}")
    def v344_stock_disclosures(market: str, code: str, months: int=3):
        try: return _dart_stock_history(code,months) if str(market).upper()=="KR" else _us_stock_history(code,months)
        except Exception as exc: return {"ok":False,"market":str(market).upper(),"code":str(code).upper(),"items":[],"status":str(exc)[:180]}
...
def status_snapshot() -> dict:
    return {"installed":_INSTALLED,"dart_market_cache":len(_DART_PAGE_CACHE),"dart_stock_cache":len(_DART_STOCK_CACHE),"dart_rss_cached":bool(_DART_RSS_CACHE),"naver_disc_cache":len(_NAVER_DISC_CACHE),"naver_bars_cache":len(_NAVER_BARS_CACHE),"index_cache":len(_INDEX_CACHE),"version":"v34.8"}
