...
def _inject(path: str, *, css: bool = True, scripts: tuple[str, ...] = ()):
    ...
    if css and "/static/v34.css" not in text:
        text = text.replace("</head>", '  <link rel="stylesheet" href="/static/v34.css?v=34">\n</head>')
    for script in scripts:
        if script not in text:
            text = text.replace("</body>", f'  <script src="{script}?v=34"></script>\n</body>')
...
def _recent_volume_surge(q) -> tuple[float, float, float]:
    now = time.time()
    hist = [(float(ts), float(vol or 0)) for ts, _px, vol in list(getattr(q, "tick_history", []) or []) if float(ts) >= now - 70]
    recent = 0.0
    if len(hist) >= 2:
        hist.sort(key=lambda x: x[0])
        diffs = [max(0.0, hist[i][1] - hist[i - 1][1]) for i in range(1, len(hist))]
        recent = sum(diffs)
        if recent <= 0:
            recent = max(v for _, v in hist) - min(v for _, v in hist)
    bars = [b for b in list(getattr(q, "daily_bars", []) or []) if float(b.get("volume") or 0) > 0]
    prev5 = bars[-5:]
    avg5 = statistics.fmean(float(b.get("volume") or 0) for b in prev5) if prev5 else 0.0
    expected_1m = avg5 / 390.0 if avg5 > 0 else 0.0
    ratio = recent / expected_1m if expected_1m > 0 else 0.0
    return ratio, recent, avg5
...
def _flow_priority(q) -> dict:
    ratio, recent_vol, avg5 = _recent_volume_surge(q)
    strength, accel = _execution_accel(q)
    trend = _five_minute_trend("KR", q.code)

    volume_score = _clamp((ratio - 2.0) / 6.0 * 30.0, 0, 30)
...
def _detect_flow_alerts(scalp_candidates):
    ...
    by_code = {x.get("code"): x for x in (scalp_candidates or [])}
    new_rows = []
...
def rebuild_cache_v34(market, now=None):
    ...
    if market == "KR":
        ...
        # Alerted names get first analysis/trading priority only after preserving
        # the 72-point gate so a weak alert cannot block valid candidates.
        scalp.sort(
            key=lambda x: (
    ...
    for quote in core.feed.quotes_for(market).values():
        ...
        if needle in code.lower() or needle in name.lower():
            ...
            rows.append({"market": market, "code": code, "name": name,
                         "price": quote.price, "sector": core.sector_name(quote, market),
                         "starts": starts})
    rows.sort(key=lambda x: (not x["starts"], x["name"], x["code"]))
    for x in rows:
        x.pop("starts", None)
    return {"market": market, "items": rows[:20]}
