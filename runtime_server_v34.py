...
# ---------------------------------------------------------------------------
# v34.7 UI ownership cleanup
# ---------------------------------------------------------------------------
# runtime_server (v33 compatibility) injects v33.css/v33.js into the main page
# as soon as it is imported.  On v34 this leaves two UI owners fighting over
# the same DOM and, on mobile, the legacy CSS wins because it is appended last.
# Remove those main-page legacy assets here.  Coin pages keep their v33 layer.
def _strip_asset(path: str, asset: str):
    p = Path(path)
    if not p.exists():
        return
    text = p.read_text(encoding="utf-8")
    if asset.endswith(".css"):
        text = re.sub(rf'\s*<link[^>]+href=["\'][^"\']*{re.escape(asset)}(?:\?[^"\']*)?["\'][^>]*>\s*', '\n', text, flags=re.I)
    else:
        text = re.sub(rf'\s*<script[^>]+src=["\'][^"\']*{re.escape(asset)}(?:\?[^"\']*)?["\'][^>]*>\s*</script>\s*', '\n', text, flags=re.I)
    p.write_text(text, encoding="utf-8")

_strip_asset("static/index.html", "/static/v33.css")
_strip_asset("static/index.html", "/static/v33.js")
# Detail pages are owned only by v346.css + their own JS.
_strip_asset("static/stock.html", "/static/v34.css")
_strip_asset("static/index-detail.html", "/static/v34.css")
...
def _inject(path: str, *, css: bool = True, extra_css: tuple[str, ...] = (), head_scripts: tuple[str, ...] = (), scripts: tuple[str, ...] = ()):
    p = Path(path)
    if not p.exists():
        return
    text = p.read_text(encoding="utf-8")
    asset_version = (os.getenv("RENDER_GIT_COMMIT") or os.getenv("GY_BUILD_ID") or str(int(time.time())))[:12]
    def inject_css(asset):
        nonlocal text
        if asset not in text:
            text = text.replace("</head>", f'  <link rel="stylesheet" href="{asset}?v={asset_version}">\n</head>')
        else:
            text = re.sub(re.escape(asset) + r'(?:\?v=[^"\']+)?', f'{asset}?v={asset_version}', text)
    if css:
        inject_css("/static/v34.css")
    for asset in extra_css:
        inject_css(asset)
    for script in head_scripts:
        tag=f'<script src="{script}?v={asset_version}"></script>'
...
# v34.5: one owner per screen.
# - Main page: base app.js owns state/candidate visibility, v34.js owns utilities,
#   v344.js owns session/default-market/event browser only.
# - Stock page: stock.js owns chart, flow and disclosures. No legacy overlay scripts.
# - Coin keeps the original wine palette.
_inject("static/index.html", css=False, extra_css=("/static/v34.css", "/static/v344.css", "/static/v346.css"), scripts=("/static/v34.js", "/static/v344.js"))
_inject("static/coin.html", scripts=("/static/coin-v34.js",))
_inject("static/coin-detail.html", scripts=("/static/coin-detail-v34.js", "/static/v343-chart.js"))
_inject("static/stock.html", css=False, extra_css=("/static/v346.css",), scripts=("/static/stock.js",))
_inject("static/index-detail.html", css=False, extra_css=("/static/v346.css",), scripts=("/static/index.js",))
...
_SEARCH_MASTER_READY: dict[str, bool] = {"KR": False, "US": False}
_SEARCH_MASTER_LAST_ATTEMPT: dict[str, float] = {"KR": 0.0, "US": 0.0}
...
def _search_norm(value: str) -> str:
    # Search should survive spaces/hyphens and Korean/English case differences.
...
def _ensure_search_catalog(market: str) -> dict[str, dict]:
    ...
    with _SEARCH_MASTER_LOCK:
        catalog = _SEARCH_CATALOG.setdefault(market, {})
        # A previous failed master load used to leave a few live quotes in the
        # catalog forever.  Only a successfully loaded master may stop retries.
        if _SEARCH_MASTER_READY.get(market):
            return catalog
        if catalog and now - float(_SEARCH_MASTER_LAST_ATTEMPT.get(market, 0) or 0) < 30:
            return catalog
        _SEARCH_MASTER_LAST_ATTEMPT[market] = now
        ...
        try:
            if market == "KR":
                ...
                for row in _frame_rows(load_master("m_gtsstock")):
                    ...
                    if not code or len(code) > 16 or not re.fullmatch(r"[A-Z0-9.\-]+", code):
                        continue
                    name = str(
                    ...
                    master_rows.setdefault(code, {
                        "market": "US", "code": code, "name": name or code,
                        "sector": industry or "미국주식",
                    })
            if master_rows:
                catalog.update(master_rows)
                _SEARCH_MASTER_READY[market] = True
                _SEARCH_MASTER_ERROR[market] = ""
            else:
                _SEARCH_MASTER_ERROR[market] = "종목 마스터 응답이 비어 있음"
        except Exception as exc:
            _SEARCH_MASTER_ERROR[market] = str(exc)[:240]

        # Preserve already-warmed symbols even while the full master is retrying.
...
def _track_search_stock(market: str, code: str) -> dict:
    ...
    if meta is None:
        # Give the master one explicit retry before declaring an exact code bad.
        with _SEARCH_MASTER_LOCK:
            _SEARCH_MASTER_READY[market] = False
            _SEARCH_MASTER_LAST_ATTEMPT[market] = 0.0
        meta = _ensure_search_catalog(market).get(code)
    if meta is None:
    ...
    # Never write a historical close into the live quote object.  The detail
    # endpoint can display the last official close separately while trading
    # freshness checks continue to see a genuinely missing/stale live quote.
...
def v34_search(market: str = "KR", q: str = ""):
    market = core.normalize_market(market)
    if market not in ("KR", "US"):
        return {"ok": False, "market": market, "items": [], "error": "stock market only"}
    raw = str(q or "").strip()
    needle = _search_norm(raw)
    if not needle:
        return {"ok": True, "market": market, "items": []}
    catalog = _ensure_search_catalog(market)
    live = core.feed.quotes_for(market)
    rows = []
    for code, meta in catalog.items():
        name = str(meta.get("name") or code)
        code_n, name_n = _search_norm(code), _search_norm(name)
        if needle not in code_n and needle not in name_n:
            continue
        quote = live.get(code)
        exact = needle in (code_n, name_n)
        starts = code_n.startswith(needle) or name_n.startswith(needle)
        rows.append({
            ...
            "tracked": bool(quote), "_rank": (0 if exact else 1 if starts else 2),
        })
    rows.sort(key=lambda x: (x["_rank"], len(x["name"]), x["name"], x["code"]))
    for x in rows:
        x.pop("_rank", None)
    return {
        "ok": True, "market": market, "items": rows[:40], "catalog_count": len(catalog),
        "master_ready": bool(_SEARCH_MASTER_READY.get(market)),
...
import disclosure_v342
disclosure_v342.install(globals())

import v343_features
v343_features.install(globals())

import v344_features
v344_features.install(globals())


@core.app.middleware("http")
async def v344_no_stale_ui(request, call_next):
    response = await call_next(request)
    path = request.url.path
    if path == "/" or path.startswith(("/stock/", "/index/", "/coin")):
        response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
        response.headers["Pragma"] = "no-cache"
        response.headers["Expires"] = "0"
    elif path.startswith(("/static/v34", "/static/v344", "/static/v346", "/static/stock.js", "/static/index.js")):
...
def _v347_diag():
    ...
    return {
        "dart_configured": bool(str(getattr(core.events, "api_key", "") or "").strip()),
        "dart_live_events": len(list(getattr(core.events, "events", []) or [])),
        "search_kr_master_ready": bool(_SEARCH_MASTER_READY.get("KR")),
        "search_kr_catalog": len(_SEARCH_CATALOG.get("KR", {})),
        "samsung_price": float(getattr(q, "price", 0) or 0),
        "samsung_daily_bars": len(list(getattr(q, "daily_bars", []) or [])),
        "nh_kr_connected": bool(getattr(core.feed, "connected", {}).get("KR")),
        "nh_kr_error": str(getattr(core.feed, "errors", {}).get("KR") or "")[:120],
    }

@core.app.get("/api/v34/status")
def v34_status():
    global _STATUS_DIAG_AT
    diag = _v347_diag()
    now = time.time()
    if now - _STATUS_DIAG_AT >= 30:
        _STATUS_DIAG_AT = now
        print("V347 DIAG " + " ".join(f"{k}={v}" for k, v in diag.items()), flush=True)
    return {
        "ok": True,
        "version": "v34.7",
        "diagnostics": diag,
        ...
        "disclosure": disclosure_v342.status_snapshot(),
        "v343": v343_features.status_snapshot(),
        "v344": v344_features.status_snapshot(),
